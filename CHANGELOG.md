...
### Changed
- **Refactor**: improved type safety across `client.ts`, `resources.ts`, `tools.ts`, and `prompts.ts`; extracted shared types into `src/types.ts` to eliminate duplication (#116)
- **Tests**: new `cache.test.ts` with full unit coverage of cache internals; expanded `index.test.ts` coverage from 87% to 97%+ across all metrics (#116)

### Fixed
- Updated vulnerable dependencies to latest patched versions (#116)
- Added Node.js 24 to CI test matrix (#116)
...
### Fixed
- Docker: use `npm ci --ignore-scripts` so TypeScript build step doesn't run before source files are copied
- Docker: remove `tsconfig.json` from `.dockerignore` (required for TypeScript compilation)
- README: link Docker image badge to GHCR package page
